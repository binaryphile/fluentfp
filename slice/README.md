...
- **Transform**: `Convert`, `FlatMap`, `Map` (`MapperTo`), `Reverse`, `Shuffle`, `Intersperse`, `ToString`, `ToInt`, other `To*`, `Clone`, `Unique` (`String`), `UniqueBy`, `Samples`, `SortBy`, `SortByDesc`, `IsSorted`, `IsSortedBy`
- **Combine**: `Zip`, `ZipWith`
