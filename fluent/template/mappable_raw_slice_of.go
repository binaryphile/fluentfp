...
// To{Type}s applies the provided function to each element of the slice, mapping it to a Slice of type {type}.
func (ts MappableRawSliceOf[T, R]) To{Type}s(fn func(T) {type}) MappableSliceOf[{type}, R] {
	return Map(ts, fn)
}
