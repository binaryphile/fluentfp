...
// MappableRawSlice derives from slice and has a type parameter R that is used solely to specify the return type of the Map method.
type MappableRawSlice[T any, R any] []T
...
func (ts MappableRawSlice[T, R]) Convert(fn func(T) T) MappableRawSlice[T, R] {
	return Map(ts, fn)
}

// KeepIf returns a new slice containing only the elements for which the provided function returns true.
func (ts MappableRawSlice[T, R]) KeepIf(fn func(T) bool) MappableRawSlice[T, R] {
...
func (ts MappableRawSlice[T, R]) MapWith(fn func(T) R) RawSliceOf[R] {
	return Map(ts, fn)
}

// RemoveIf returns a new slice containing only the elements for which the provided function returns false.
func (ts MappableRawSlice[T, R]) RemoveIf(fn func(T) bool) MappableRawSlice[T, R] {
	results := make([]T, 0, len(ts))
	for _, t := range ts {
		if !fn(t) {
			results = append(results, t)
...
func (ts MappableRawSlice[T, R]) TakeFirst(n int) MappableRawSlice[T, R] {
	if n > len(ts) {
		n = len(ts)
	}
...
func (ts MappableRawSlice[T, R]) ToComplex64OptionsWith(fn func(T) option.Complex64) MappableSliceOf[option.Complex64, R] {
	return Map(ts, fn)
}
...
func (ts MappableRawSlice[T, R]) ToComplex64SlicesWith(fn func(T) []complex64) MappableRawSlice[[]complex64, R] {
	return Map(ts, fn)
}
...
func (ts MappableRawSlice[T, R]) ToComplex128SlicesWith(fn func(T) []complex128) MappableRawSlice[[]complex128, R] {
	return Map(ts, fn)
}

// ToByteSlicesWith applies the provided function to each element of the slice, mapping it to a slice of byte slices.
func (ts MappableRawSlice[T, R]) ToByteSlicesWith(fn func(T) []byte) MappableRawSlice[[]byte, R] {
	return Map(ts, fn)
}

// ToRuneSlicesWith applies the provided function to each element of the slice, mapping it to a slice of rune slices.
func (ts MappableRawSlice[T, R]) ToRuneSlicesWith(fn func(T) []rune) MappableRawSlice[[]rune, R] {
	return Map(ts, fn)
}
