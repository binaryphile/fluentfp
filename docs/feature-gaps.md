...
| Contains | Membership check for any `comparable` | lo, underscore, fp-go | `String.Contains` for strings only; `.Any(eq)` for others | Standalone — needs `T comparable` constraint | Add |
...
| Every/None | All/no elements match predicate | lo, underscore, fp-go | `!Any(pred)` for None; no direct Every | Method (terminal) — returns bool, same as `.Any()` | Skip |
...
### Recommended to Add (4)

**Contains** — Generic membership check returning bool. Currently only `String.Contains` exists. Must be a standalone function: `Contains[T comparable](ts []T, target T) bool` — cannot be a method on `Mapper[T any]` because `T` isn't constrained to `comparable`. High usage in lo importers; the `.Any(eq)` workaround requires constructing an equality closure each time.
...
**Every/None** — `!slice.From(ts).Any(pred)` for None. For Every, negate the predicate: `!slice.From(ts).Any(notPred)`. Slightly awkward but functional. If Contains is added, Every/None become even less necessary (Contains covers the most common "check all" pattern).
