...
func main() {
	...
	fmt.Println("notOkBool.OrFalse():", notOkBool.OrFalse())
	...
	fmt.Println("eightyFourOption.OrZero():", eightyFourOption.OrZero())

	// ToString maps to string
	fortyTwoStrOption := fortyTwoOption.ToString(strconv.Itoa)
	fmt.Println("fortyTwoStrOption.OrEmpty():", fortyTwoStrOption.OrEmpty())
	...
	if unknownFortyTwo, ok := userFromFortyTwoOption.Get(); ok {
		fmt.Println("unknownFortyTwo:", unknownFortyTwo)
	}

	// === Checking and Side Effects ===

	// Call executes a function only if ok
	// printFortyTwo prints the value.
	printFortyTwo := func(i int) { fmt.Println("called with:", i) }
	fortyTwoOption.Call(printFortyTwo)
	...
	fmt.Println("adultOption.IsOk():", adultOption.IsOk())

	// ToNotOkIf makes not-ok if predicate passes
	notAdultOption := fortyTwoOption.ToNotOkIf(isAdult)
	fmt.Println("notAdultOption.IsOk():", notAdultOption.IsOk())

	// ToOpt converts back to pointer (inverse of IfNotNil)
	fmt.Println("fortyTwoOption.ToOpt():", *fortyTwoOption.ToOpt())
}
