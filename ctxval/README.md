...
reqID := ctxval.From[RequestID](ctx)  // Option("req-123")
trID  := ctxval.From[TraceID](ctx)    // Option("trace-abc")
```

```go
// Comma-ok extraction when you need to branch
if user, ok := ctxval.From[User](ctx).Get(); ok {
...
**Type-keyed** (`With`/`From`): When each Go type naturally maps to one value per context. Middleware injecting a `User`, `RequestID`, or `TraceID` — one value per type, no collision possible.
...
- **Retrieve**: `From[T](ctx)` — returns `Option[T]`; `Key.From(ctx)` — returns `Option[T]`
- **Keys**: `NewKey[T]()` — create unique named key (pointer identity)

`From` returns an `Option`, so all option operations apply: `.Or("default")`, `.Get()`, `.IsOk()`, `.IfOk(fn)`.
