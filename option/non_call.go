...
// NonZeroCall returns an ok option of fn(t) provided that t is not the zero value for T, or not-ok otherwise.
...
// NonEmptyCall returns an ok option of fn(s) provided that s is not empty, or not-ok otherwise.
// It is the string-specific variant of NonZeroCall.
...
// NonNilCall returns an ok option of fn(*t) provided that t is not nil, or not-ok otherwise.
